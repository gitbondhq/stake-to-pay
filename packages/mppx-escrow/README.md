...
`MPPEscrowAbi` is checked into `src/abi/MPPEscrow.ts`. It is generated by
`scripts/generate-abi.mjs` from the local `out/MPPEscrow.sol/MPPEscrow.json`
artifacts so we keep our local ABI generation flow in this repo.
